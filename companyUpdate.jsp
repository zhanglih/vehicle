...
<html>
	...
	<body onLoad="JavaScript:form.namex.focus()">
		<form name="form" method="post" action="companyUpdateResult.jsp?id=<%=ID%>" onsubmit="return verify()">
		<!-- Wrapper -->
			<div id="wrapper">
								...
								<section>
									...
									<div class="row uniform">
										...
										<div class="7u 12u$(xsmall)">
											<input type="text" name="namex" id="namex" value="<%=companySchema.Namex%>" placeholder="公司名称" maxlength="50" data-required="true"/>
										</div>

										<div class="12u$"><label for="contactPerson">联系人</label></div>
										<div class="7u 12u$(xsmall)">
											<input type="text" data-required name="contactPerson" value="<%=companySchema.ContactPerson%>" placeholder="联系人"  maxlength="30"/>
										</div>
										
										<div class="12u$"><label for="tel">电话</label></div>
										<div class="7u 12u$(xsmall)">
											<input type="text" name="tel" value="<%=companySchema.Tel%>" placeholder="电话" maxlength="20" />
										</div>

										<div class="12u$"><label for="mobile">手机</label></div>
										<div class="7u 12u$(xsmall)">
											<input type="text" data-required name="mobile" value="<%=companySchema.Mobile%>" maxlength="20" onkeypress="return /^[0-9]*$/.test(String.fromCharCode(event.which))" placeholder="手机" />
										</div>
										
										<div class="12u$"><label for="email">电子邮箱</label></div>
										<div class="7u 11u$(xsmall)">
											<input type="text" data-required name="email" id="email" maxlength="30" value="<%=companySchema.Email%>" onkeypress="return /[0-9a-zA-Z]|[@]|\./.test(String.fromCharCode(event.which))" placeholder="电子邮箱" />
										</div>
										
										<div class="12u$"><label for="address">地址</label></div>
										<div class="7u 12u$(xsmall)">
											<input type="text" name="address" value="<%=companySchema.Address%>" maxlength="200" placeholder="地址" />
										</div>

										<div class="12u$"><label for="postcode">邮政编码</label></div>
										<div class="7u 12u$(xsmall)">
											<input type="text" name="postcode" value="<%=companySchema.Postcode%>" maxlength="10" placeholder="邮政编码" />
										</div>

										<div class="12u$"><label for="website">网站</label></div>
										<div class="7u 12u$(xsmall)">
											<input type="text" name="website" value="<%=companySchema.Website%>" maxlength="200" placeholder="网站" />
										</div>

										<div class="12u$"><label for="isEnabled">启用</label></div>
			                            <%
			                                if(companySchema.IsEnabled==1)
			                                {
			                            %>
										...
										<div class="7u 12u$(xsmall)">
											<textarea name="memo" rows="6"><%=companySchema.Memo%></textarea>
										</div>
										<!-- Break -->
										<div class="12u$">
											<ul class="actions">
